<template>
  <div class="content">
    ...
    >
      ...
      <span style="font-size: 18px">Update Galeri Program Pelatihan LPK</span>
    </div>
    <form
      ref="form"
      novalidate
        ...
        <md-card-header>
          <div class="md-title">
            <h2 style="letter-spacing: 2px;color: green;margin-top:30px;">
              <strong>Update Galeri Program Pelatihan LPK</strong>
            </h2>
          </div>
        </md-card-header>
...
export default {
  ...
  computed: {
    ...
    ...mapState("gallery", ["galleryData", "galleryList"]),
    headerStyle() {
      return {
  ...
  methods: {
    ...
    ...mapActions("gallery", ["getGallery", "updateGallery"]),
    async onFetchData() {
      await this.getGallery({ id: this.$route.params.id });
