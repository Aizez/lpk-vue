<template>
  <div class="content">
    <div style="margin-top: 50px;">
      ...
      <span style="font-size: 18px">Update User</span>
    </div>
    <form ref="form" novalidate class="md-layout" @submit.prevent="onSubmit">
      <md-card
        class="md-layout-item md-size-95 md-small-size-100 mx-auto"
        style="padding: 0px 50px 20px 50px; margin-left: 30px"
      >
        <md-card-header>
          <div class="md-title">
      ...
      <md-snackbar :md-active.sync="userSaved"
        >User Berhasil Diupdate!</md-snackbar
      >
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions("user", ["getUsers", "updateUsers"]),
    async onFetchData() {
      await this.getUsers({ id: this.$route.params.id });
    },
    async onSubmit() {
      this.$v.$touch();
      this.sending = true;
